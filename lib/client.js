...
var inject = ["remote", "locale"];
var NS = "dshReadingPad";
var zh = {
  title: "\u9605\u8BFB\u677F",
...
var PREFS_KEY = "dsh-reading-pad.prefs.v1";
var SCROLL_KEY = "dsh-reading-pad.scroll.v1";
...
var ReadingBoundary = class extends React.Component {
  ...
  componentDidCatch(error, info) {
    console.error("[dsh-reading-pad]", error, info?.componentStack);
  }
  render() {
  ...
  function registerReadingTab() {
    ...
    try {
      bs.registerTab({
        id: "dsh-reading-pad",
        title: () => t("title"),
        icon: React.createElement("span", { style: { fontSize: 14 } }, "\u{1F4D6}"),
        single: true,
        order: 30,
        badge: () => {
          try {
        ...
        onActivate: () => {
          try {
            browserStorage()?.setItem("dsh-reading-pad.seen-revision.v1", String(padState.latestRevision));
          } catch {
          }
        },
