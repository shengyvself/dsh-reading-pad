...
**Requires DSH 0.1.5-rc.5 or newer.** 使用官方原生右侧栏 keyed 槽位（`sidebar.right.pane.tab`），**不依赖** `dsh-better-sidebar`。
...
安装插件**不会**升级宿主 DSH。如果你用的是更老的 DSH 版本（`0.1.0-rc.*` 或更早），原生右侧栏 keyed 槽位可能不存在——tab 不会出现在「+」菜单里。升级 DSH 即可解决。
